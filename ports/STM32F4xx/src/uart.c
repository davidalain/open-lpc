...
uint32_t uart_write(const uart_t *uart, const uint8_t *data, uint32_t length) {
	...
	for (i = 0; i < length; i++) {
		while ((usart_typedef->SR & (1 << 6)) == 0)
			; // Espera terminar de enviar
...
void uart_enable_interrupt(const uart_t *uart, uint16_t usart_int) {
	...
	USART_TypeDef *usart_typedef = (USART_TypeDef *) uart->uart;
	uint32_t int_mask = (((uint16_t) 0x01) << (usart_int & ((uint16_t) 0x001F)));

	if (!uart || !uart->uart)
		return;

	switch ((((uint8_t) usart_int) >> 0x05) /*control register num*/) {
	case 0x01:
		usart_typedef->CR1 |= int_mask;
		break;
	case 0x02:
		usart_typedef->CR2 |= int_mask;
		break;
	case 0x03:
	default:
		usart_typedef->CR3 |= int_mask;
		break;
	}
...
void uart_set_interrupt_handler(const uart_t *uart, void (*interrupt)(void)) {
	...
	switch ((uint32_t) uart->uart) {
	case USART1_BASE:
		index = USART1_IRQn + 0x10;
		break;
	case USART2_BASE:
		index = USART2_IRQn + 0x10;
		break;
	case USART3_BASE:
		index = USART3_IRQn + 0x10;
		break;
	case UART4_BASE:
		index = UART4_IRQn + 0x10;
		break;
	case UART5_BASE:
		index = UART5_IRQn + 0x10;
		break;
	case USART6_BASE:
		index = USART6_IRQn + 0x10;
		break;
	default:
		//Ponteiro uart ainda não foi configurado
		//Nada deve ser feito
		return;
	}

	g_pfnVectors[index] = interrupt;
